...
int testCve() {
    ...
    /* GET */
    SmartPointer<char> firesight_json(worker.cve(firesightPath).src_firesight_json.get());
    ...
    assert(worker.cve(savePath).src_save_fire.isFresh());
    /*GET*/
    SmartPointer<char> save_fire(worker.cve(savePath).src_save_fire.get());
    assert(testString("save.fire GET", "{}",save_fire));
    assert(testNumber((size_t) 0, worker.cve(savePath).src_saved_png.peek().size()));
    assert(!worker.cve(savePath).src_save_fire.isFresh());
    /*ASYNC*/
    assert(testProcess(01020)); // monitor + save
    assert(worker.cve(savePath).src_save_fire.isFresh());
    ...
    /*ASYNC*/
    assert(testProcess(00)); // camera + mat_gray
    assert(testNumber((size_t) 43249, worker.cameras[0].src_monitor_jpg.peek().size()));
    ...
    assert(testNumber((size_t)43249, worker.cameras[0].src_output_jpg.peek().size()));
    /*GET*/
    SmartPointer<char> process_fire_json(worker.cve(processPath).src_process_fire.get());
    assert(testNumber((size_t)43249, worker.cameras[0].src_output_jpg.peek().size()));
    assert(testString("process.fire GET", "{}",worker.cve(processPath).src_process_fire.peek()));
    assert(!worker.cve(processPath).src_process_fire.isFresh());
    /*ASYNC*/
    assert(testProcess(01010)); // monitor + process + camera + mat_gray
    assert(testNumber((size_t) 43940, worker.cameras[0].src_output_jpg.peek().size()));
    assert(testNumber((size_t) 43940, worker.cameras[0].src_monitor_jpg.peek().size()));
    /*ASYNC*/
    assert(testProcess(00)); // monitor + camera + mat_gray
    assert(testNumber((size_t) 43940, worker.cameras[0].src_monitor_jpg.peek().size()));
    assert(worker.cve(processPath).src_process_fire.isFresh());
...
int testSpiralSearch() {
    try {
        ...
        four.setScale(1,10);
        assert(testNumber( 0l, (long)four.getX()));
        assert(testNumber( 0l, (long)four.getY()));
        assert(four.next());
        assert(testNumber( 1l, (long)four.getX()));
        assert(testNumber( 0l, (long)four.getY()));
        assert(four.next());
        assert(testNumber( 1l, (long)four.getX()));
        assert(testNumber( 10l, (long)four.getY()));
        assert(four.next());
        assert(testNumber( 0l, (long)four.getX()));
        assert(testNumber( 10l, (long)four.getY()));
        assert(four.next());
        ...
        assert(testNumber( -10l, (long)four.getY()));
        assert(four.next());
        assert(testNumber( 2l, (long)four.getX()));
        assert(testNumber( 0l, (long)four.getY()));
        assert(four.next());
        assert(testNumber( 2l, (long)four.getX()));
        assert(testNumber( 10l, (long)four.getY()));
        assert(four.next());
        assert(testNumber( 2l, (long)four.getX()));
        assert(testNumber( 20l, (long)four.getY()));
        assert(four.next());
        assert(testNumber( 1l, (long)four.getX()));
        assert(testNumber( 20l, (long)four.getY()));
        assert(four.next());
        ...
        assert(testNumber( 0l, (long)fourX.getX()));
        assert(testNumber( 10l, (long)fourX.getY()));
        assert(fourX.next());
        assert(testNumber( 1l, (long)fourX.getX()));
        assert(testNumber( 10l, (long)fourX.getY()));
        assert(fourX.next());
        assert(testNumber( 2l, (long)fourX.getX()));
        assert(testNumber( 10l, (long)fourX.getY()));
        assert(!fourX.next());

        SpiralIterator fourY(1, 3);
        fourY.setOffset(10, 1);
        assert(testNumber( 10l, (long)fourY.getX()));
        assert(testNumber( 0l, (long)fourY.getY()));
        assert(fourY.next());
        assert(testNumber( 10l, (long)fourY.getX()));
        assert(testNumber( 1l, (long)fourY.getY()));
        assert(fourY.next());
        assert(testNumber( 10l, (long)fourY.getX()));
        assert(testNumber( 2l, (long)fourY.getY()));
        assert(!fourY.next());
