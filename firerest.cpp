...
string FireREST::config_cnc_serial(string dcePath, json_t *pSerial) {
	...
	dce.set_serial_ack(config_string(pSerial, "ack", "{\"sr\"").c_str());	// TinyG acknowledgement 
...
string FireREST::config_dce(string dcePath, json_t *jdce) {
	...
	json_t * jgcode = json_object_get(jdce, "gcode");
	dce.set_gcode_finish(config_string(jgcode, "finish", "G4P0").c_str());
	LOGINFO2("FireREST::config_dce(%s) gcode finish:%s", dcePath.c_str(), dce.get_gcode_finish());
...
char * FireREST::configure_path(const char *path) {
    ...
    if (bytesRead != length) {
        LOGERROR3("FireREST::configure_path(%s) configuration file size expected:%ldB actual:%ldB", 
