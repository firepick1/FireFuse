...
typedef class CVE {
    private:
        string name;                                        //
    private:
        bool _isColor;                                      // TRUE if CVE is a color endpoint, or FALSE if endpoint is grayscale

    public:
        LIFOCache<SmartPointer<char> > src_saved_png;        // Pointer to https://github.com/firepick1/FireREST/wiki/saved.png
    public:
        LIFOCache<SmartPointer<char> > src_save_fire;        // Pointer to https://github.com/firepick1/FireREST/wiki/save.fire (grab saved.png when accessed)
    public:
        LIFOCache<SmartPointer<char> > src_process_fire;     // Pointer to https://github.com/firepick1/FireREST/wiki/process.fire
    public:
        LIFOCache<SmartPointer<char> > src_firesight_json;   // Pointer to https://github.com/firepick1/FireREST/wiki/firesight.json
    public:
        LIFOCache<SmartPointer<char> > src_properties_json;  // Pointer to https://github.com/firepick1/FireREST/wiki/properties.json
    public:
        static string cve_path(const char *pPath);          // String containing path to Computer Vision Endpoint
    public:
        CVE(string name);                                    // Constructor
    public:
        ~CVE();                                              // Destructor
    public:
    ...
    public:
        int save(BackgroundWorker *pWorker);
    public:
        int process(BackgroundWorker *pWorker);
    public:
        inline bool isColor() {
            return _isColor;    // TRUE if CVE is a color endpoint, or FALSE if endpoint is grayscale
        }
} CVE, *CVEPtr;
...
typedef class DCE {
    ...
    private:
        pthread_t tidReader;
    public:
        vector<string> serial_device_config;
    private:
        char *jsonBuf;
    private:
        int activeRequests;
    ...
    private:
        int jsonDepth;
    private:
        string gcode_finish;	// Complete current operations and await further instructions (e.g., G4P0)
    private:
        char *inbuf;
    private:
        int inbuflen;
    private:
        int inbufEmptyLine;
    private:
        string serial_reader_buf;
    private:
        int serial_send_eol(const char *buf, size_t bufsize);
    private:
        int serial_send(const char *data, size_t length);
    private:
        int serial_read_char(int c);
    private:
        int post_serial_status(const char *line);
    private:
    ...
    protected:
        ...
        // Common data
    public:
        LIFOCache<SmartPointer<char> > snk_gcode_fire;
    public:
        LIFOCache<SmartPointer<char> > src_gcode_fire;
        //public: LIFOCache<SmartPointer<char> > src_properties_json;

    public:
        static vector<std::string> gcode_lines(const string &gcode);
    public:
        static string dce_path(const char *pPath);
    public:
        DCE(string name);
    public:
        ~DCE();
    ...
    public:
        inline const char * get_gcode_finish() {
            return gcode_finish.c_str();
        }
	public:
		inline void set_serial_ack(const char * value="ok") {
			serial_ack = value;
		}
	public:
		inline const char *get_serial_ack() {
			return serial_ack.c_str();
		}
    public:
        int serial_init();
    public:
    ...
    public:
        int gcode(BackgroundWorker *pWorker);
    public:
        inline string getSerialStty() {
            return serial_stty;
        }
    public:
        inline void setSerialStty(const char * value) {
            serial_stty = value;
        }
    public:
        inline string getSerialPath() {
            return serial_path;
        }
    public:
        inline void setSerialPath(const char * value) {
            serial_path = value;
        }
    public:
        inline vector<string> getSerialDeviceConfig() {
            return serial_device_config;
        }
} DCE, *DCEPtr;
...
typedef class CameraNode {
    private:
        double camera_idle_capture_seconds;
    private:
        double camera_seconds; // time of last camera update
    private:
        double output_seconds; // time of last FireSight pipeline completion
    private:
        double monitor_duration; // number of seconds to show last output
    private:
        int min_capture_ms; // minimum number of milliseconds between captures
    private:
        bool captureActive;
    private:
        pid_t raspistillPID;
    private:
        long msCapture; // earliest time of next capture

        // Common data
    public:
        LIFOCache<SmartPointer<char> > src_camera_jpg;
    public:
        LIFOCache<Mat> src_camera_mat_gray;
    public:
        LIFOCache<Mat> src_camera_mat_bgr;
    public:
        LIFOCache<SmartPointer<char> > src_monitor_jpg;
    public:
        LIFOCache<SmartPointer<char> > src_output_jpg;
    ...
    public:
        ~CameraNode();
    public:
        void init();                                     //Inits camera.  Starts Raspistill, etc..
    public:
        void clear();
    public:
        void endCapture();
    public:
        int async_update_camera_jpg();
    public:
        int accept_new_image(SmartPointer<char> jpg);
    public:
        int async_update_monitor_jpg();
    public:
        void setOutput(Mat image);
    public:
        inline int get_min_capture_ms() {
            return min_capture_ms;
        }
    public:
        void set_min_capture_ms(int value = 500);
} CameraNode;
...
typedef class BackgroundWorker {
    private:
        double idle_period; // minimum seconds between idle() execution. Gets set by config.json.
    private:
        std::map<string, CVEPtr> cveMap;
    private:
        std::map<string, DCEPtr> dceMap;
    private:
        std::map<string, DCEPtr> serialMap;
    private:
        double idle_seconds; // time of last idle() execution
    private:
        int async_save_fire();
    private:
        int async_process_fire();
    private:
        int async_gcode_fire();
    ...
    public:
        ~BackgroundWorker();
    public:
        static int callSystem(char *cmdbuf); //Execute a system shell command
    public:
        CVE& cve(string path, bool create=FALSE);
    public:
        DCE& dce(string path, bool create=FALSE);
    public:
        vector<string> getCveNames();
    public:
        vector<string> getDceNames();
    public:
        void clear();
    public:
        void process();
    public:
        inline void setIdlePeriod(double value) {
            idle_period = value;
        }
    public:
        inline double getIdlePeriod() {
            return idle_period;
        }
    public:
        inline DCEPtr getSerialDCE(string serialPath) {
            return serialMap[serialPath];
        }
    public:
        inline void setSerialDCE(string serialPath, DCEPtr pDce) {
            serialMap[serialPath] = pDce;
        }
...
typedef class JSONFileSystem {
    private:
        std::map<string, json_t *> dirMap;
    private:
        std::map<string, json_t *> fileMap;
    private:
        json_t *resolve_file(const char *path);
    ...
    public:
        static vector<string> splitPath(const char *path);
    public:
        void clear();
    public:
        json_t *get(const char *path);
    public:
        void create_file(const char *path, int perm);
    public:
        inline void create_file(string path, int perm) {
            create_file(path.c_str(), perm);
        }
    public:
...
typedef class FireREST {
    ...
    private:
        int processCount;
    private:
        JSONFileSystem files;
    private:
        string config_camera(const char* cv_path, json_t *pCamera, const char *pCameraName, json_t *pCveMap);
    private:
        string config_cv(const char* root_path, json_t *pConfig);
    private:
        string config_cnc(const char* root_path, json_t *pConfig);
    private:
        string config_cnc_serial(string dcePath, json_t *pSerial);
    private:
        string config_dce(string dcePath, json_t *pConfig);
    private:
        void create_resource(string path, int perm);
    ...
    public:
        int decrementProcessCount();
    public:
        char * configure_path(const char *path);
    public:
        void configure_json(const char *pJson);
    public:
        int perms(const char *path) {
            return files.perms(path);
        }
    public:
        bool isDirectory(const char *path) {
    ...
    public:
        static bool isSync(const char *path);
    public:
        vector<string> fileNames(const char *path) {
            return files.fileNames(path);
        }
} FireREST;
...
typedef class SpiralIterator { // simpler than a C++ <iterator>
    ...
    public:
        SpiralIterator(int xSteps=21, int ySteps=0);
    public:
