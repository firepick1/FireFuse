...
int CVE::save(DataFactory *pFactory) {
  ...
  if (image.rows && image.cols) {
    ...
    putText(image, "Saved", Point(5, image.rows-6), FONT_HERSHEY_SIMPLEX, 2, Scalar(255,255,255), 3);
    putText(image, "Saved", Point(6, image.rows-7), FONT_HERSHEY_SIMPLEX, 2, Scalar(0,0,0), 1);
    pFactory->cameras[0].setOutput(image);
